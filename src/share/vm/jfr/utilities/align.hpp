...
inline T align_up(T size, A alignment) {
  assert(is_power_of_2_t(alignment), err_msg("must be a power of 2: " UINT64_FORMAT, (uint64_t)alignment));

  T ret = align_up_(size, alignment);
  assert(is_aligned_(ret, alignment), err_msg("must be aligned: " UINT64_FORMAT, (uint64_t)ret));
